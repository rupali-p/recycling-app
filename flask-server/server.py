...
from account_details import (
    ...
    get_account_name_details,
    get_account_password_details,
    get_account_postcode_details,
    get_account_scancount_details,
)
import json
import numpy as np
...
def handle_upload():
    ...
    results, model_used = make_prediction(
        input_image=buf, output_img_format=OUTPUT_IMAGE_FORMAT
    )
...
def handle_login():
    ...
    print("In backend")
    login_result = login_user(uName=data["Usernane"], password=data["password"])
...
def view_result(articleNum):
    result = get_recycling_result(
        articleNum
    )  # This number should be changed to an input passed from the machine when need be
    if result != "Error":
        return result, 200
    else:
        return result, 500
...
def view_results():
    ...
    print(arl_results)
    return jsonify({"success": True, "arl_results": json.dumps(arl_results)})
